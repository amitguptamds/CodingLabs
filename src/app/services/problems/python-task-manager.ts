...
export const pythonTaskManager: Problem = {
...
Your task:
- **\`add_task(title, priority)\`** — Create and return a new task dict
- **\`complete_task(task_id)\`** — Mark a task as done
- **\`remove_task(task_id)\`** — Remove a task
- **\`list_tasks()\`** — Return all tasks as dicts
- **\`filter_by_priority(priority)\`** — Filter tasks by priority`,
    difficulty: 'Easy',
...
`
    ...
    boilerplateCode: `# Multi-file problem — edit manager.py
class TaskManager:
    ...
    examples: [
        ...
        { input: 'tm.list_tasks()', output: '[{"id": 1, ...}]', explanation: 'Returns all tasks as dicts' },
    ],
    constraints: [
